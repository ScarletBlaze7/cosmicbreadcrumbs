...
import { getStoredPermissions } from './utils/permissionManager';
import { syncUserData, getStoredAuthUser } from './utils/authManager';
...
export default function App() {
  ...
  // Check URL params for Stripe checkout redirect return & first download preview
  useEffect(() => {
    initFontSize();
    try {
      const params = new URLSearchParams(window.location.search);
      if (params.get('preview') === 'onboarding' || params.get('first_time') === 'true') {
    ...
    <div className="min-h-screen bg-[#06070f] cosmic-dot-bg text-slate-100 selection:bg-fuchsia-600 selection:text-white flex flex-col justify-between relative overflow-hidden">
      ...
      <AuthModal
        ...
        onAuthSuccess={(user) => {
          ...
          syncUserData({ profile: userProfile, membership });
        }}
      />

      {/* Location & Notification Permissions Request Modal */}
      <PermissionsModal
