...
export const AngelOracleView: React.FC<AngelOracleViewProps> = ({
  ...
  const [primaryMediaMode, setPrimaryMediaMode] = useState<'video' | 'artwork'>('video');
  const [clarificationMediaMode, setClarificationMediaMode] = useState<'video' | 'artwork'>('video');
  const [templeActiveVideo, setTempleActiveVideo] = useState<string | null>(null);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-purple-800/40 pb-5">
        <div className="space-y-1">
          <div className="flex items-center space-x-2">
            <span className="rounded-md bg-purple-900/60 px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-amber-300 border border-purple-700/60">
              Celestial Oracle
            </span>
            <span className="text-xs text-purple-300">18 Temple Guardians</span>
          </div>
          <h1 className="font-serif text-2xl sm:text-3xl font-bold tracking-tight text-slate-100 flex items-center gap-2">
            <span>Archangel Temple & Celestial Oracle</span>
            <Sparkles className="h-5 w-5 text-amber-400 animate-pulse" />
          </h1>
          <p className="text-xs sm:text-sm text-purple-200/90 max-w-2xl">
            Receive direct celestial transmissions, watch sacred angelic videos upon card draw, and invoke the 18 Temple Guardians.
          </p>
        </div>
            ...
            {primaryCard && (
              <div className="relative overflow-hidden rounded-3xl border-2 border-rose-500/40 bg-gradient-to-br from-slate-900 via-rose-950/20 to-slate-950 p-6 sm:p-7 shadow-2xl space-y-5 animate-in zoom-in-95 duration-300">
                ...
                <div className="flex items-center justify-between border-b border-rose-900/40 pb-3">
                  ...
                  <div className="flex items-center bg-slate-950/80 p-0.5 rounded-xl border border-rose-800/40">
                    <button
                      type="button"
            ...
            {clarificationCard ? (
              <div className="relative overflow-hidden rounded-3xl border-2 border-amber-400/60 bg-gradient-to-br from-slate-900 via-amber-950/20 to-slate-950 p-6 sm:p-7 shadow-2xl space-y-5 animate-in zoom-in-95 duration-300">
                ...
                <div className="flex items-center justify-between border-b border-amber-900/40 pb-3">
                  ...
                  {/* Video / Artwork Toggle */}
                  <div className="flex items-center bg-slate-950/80 p-0.5 rounded-xl border border-amber-500/40">
                    <button
                      type="button"
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {ARCHANGEL_ROSTER.map((arch) => {
              const isVideoActive = templeActiveVideo === arch.name;
              return (
                <div
                  key={arch.name}
