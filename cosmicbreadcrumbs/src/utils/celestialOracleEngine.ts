...
import { getSunSignFromDate, getMoonPhaseInfo } from './astrologyCalc';
...
export function generateCelestialOracleResponse({
  ...
  const index = Math.abs(chatHistoryLength + lastMessage.length + (userProfile.name?.length || 0)) % dynamicWisdoms.length;
