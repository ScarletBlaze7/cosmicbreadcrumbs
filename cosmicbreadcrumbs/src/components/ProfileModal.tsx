...
export const ProfileModal: React.FC<ProfileModalProps> = ({
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      const img = new Image();
      img.onload = () => {
        // Resize to max 280x280 for fast responsive local storage
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-purple-900/60 bg-slate-900 shadow-2xl shadow-purple-950/50 animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSave} className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div className="rounded-2xl border border-purple-800/50 bg-slate-950/70 p-4 space-y-3.5 shadow-inner">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <div className="space-y-2 text-center sm:text-left flex-1">
                <p className="text-xs text-purple-200/90 leading-relaxed">
                  {formData.avatarUrl ? (
                    <>Your custom photo is active! You can upload a new photo, select a cosmic preset, or restore your <strong>{previewSunSign.name}</strong> Zodiac sign.</>
                  ) : (
            ...
            {/* CELESTIAL ARTWORK PRESETS */}
            <div className="pt-2.5 border-t border-purple-900/40">
              <div className="text-[11px] font-medium text-purple-300 mb-2 flex items-center justify-between">
                <span>Or Choose Cosmic Artwork Presets:</span>
                <span className="text-[10px] text-purple-400 font-mono">1-Click Apply</span>
              </div>
              <div className="grid grid-cols-5 gap-2">
                {/* Default Zodiac Preset */}
                <button
                  type="button"
                  onClick={() => setFormData((prev) => ({ ...prev, avatarUrl: undefined }))}
                  className={`flex flex-col items-center p-1.5 rounded-xl border transition-all ${
                    !formData.avatarUrl
                      ...
                      : 'border-purple-900/50 bg-slate-900/60 hover:border-purple-700'
                  }`}
                  title={`Default: ${previewSunSign.name} Zodiac Sign`}
                >
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-950 text-amber-300 border border-amber-400/50">
                    <ZodiacSymbolIcon sign={previewSunSign.name} size="sm" fallbackText={previewSunSign.symbol} className="scale-75" />
                  </div>
                  <span className="text-[9px] text-amber-200 mt-1 truncate max-w-[50px] text-center font-medium">
                    {previewSunSign.name}
                  </span>
                </button>
                ...
                {CELESTIAL_PRESETS.map((preset) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span className="text-[9px] text-purple-200 mt-1 truncate max-w-[50px] text-center">
                        {preset.name.split(' ')[0]}
                      </span>
                    </button>
                  );
